document.addEventListener("DOMContentLoaded", () => {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", (event) => {
      ...
      // Store user info in session storage (for demonstration purposes)
      const userInfo = {
        email: email,
        role: currentRole,
        name:
          currentRole === "student" ? "John Smith" : currentRole === "faculty" ? "Prof. Robert Johnson" : "Admin User",
      }
  ...
  const storedUser = localStorage.getItem("eduAI_currentUser")
  if (storedUser) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", (event) => {
      ...
      // Attempt login
      if (window.EduAI && window.EduAI.login(email, password, currentRole)) {
  ...
  signupLinks.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      document.getElementById("signupForm").addEventListener("submit", (e) => {
        ...
        if (window.EduAI) {
          const result = window.EduAI.registerUser(name, email, password, signupRole)
          ...
          window.showNotification("System error. Please try again later.", "error")
        }
      })
